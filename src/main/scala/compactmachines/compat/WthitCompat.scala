...
object MachineTooltip {
  val textOffset = MinecraftClient.getInstance().textRenderer.fontHeight + 5
}
/*
...
object MachineBlockTooltip extends IBlockComponentProvider with IServerDataProvider[MachineBlockEntity] {
  override def appendBody(tooltip : ITooltip, accessor: IBlockAccessor, conf : IPluginConfig) = {
  ...
  override def appendServerData(data : NbtCompound, accessor : IServerAccessor[MachineBlockEntity], conf : IPluginConfig) = {
...
object CompactWthitConfig {
  private def rl(rl : String) = Identifier(CompactMachines.MODID, rl)
  val general = rl("general")
  val machineBlock = rl("machineblock.enabled")
}
