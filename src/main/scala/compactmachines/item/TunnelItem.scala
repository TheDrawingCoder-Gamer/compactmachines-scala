...
class TunnelItem(settings: Settings, val tunnelType : Option[TunnelType]) extends Item(settings):
  ...
  override def getName(stack: ItemStack): Text = 
    lazy val defName = Text.translatable("item.compactmachines.tunnels.tunnel")
    stack.getNbt match 
      case null => defName 
      case stackNbt => 
        stackNbt.get("type") match 
          case null => defName 
          case typeNbt => 
            TunnelType.byName(typeNbt.asString()).map((tt: TunnelType) => Text.translatable(s"item.compactmachines.tunnels.${tt.tunnelName}")).getOrElse(defName)
