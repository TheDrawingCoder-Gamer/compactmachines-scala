...
object PSDScreen: 
  val TEXTURE: Identifier = Identifier(CompactMachines.MOD_ID, "textures/gui/psd_screen.png")
class PSDScreen(title: Text) extends Screen(title): 
  override def render(matrices: MatrixStack, mouseX: Int, mouseY: Int, delta: Float): Unit =
    ...
    textRenderer.nn.drawWithShadow(matrices,
      TranslatableText(s"${CompactMachines.MOD_ID}.psd.pages.machines.title"),
      j.toFloat+17, k.toFloat+16, Formatting.GOLD.getColorValue) 
    
    val text = MultilineText.create(textRenderer.nn, TranslatableText(s"${CompactMachines.MOD_ID}.psd.pages.machines"), 222) 
