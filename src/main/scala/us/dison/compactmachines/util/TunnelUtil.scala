  ...
  */
  ...
  def getTunnelOf(direction: Option[Direction], tunnelType: TunnelType, room: Room): Option[Tunnel] = 
    room.tunnels.find { t =>
        ...
        CompactMachines.LOGGER.info(s"${t.face.toDirection.toString} == ${direction.toString} = $dirEq")
        val ttEq = t.tunnelType == tunnelType 
        CompactMachines.LOGGER.info(s"${t.tunnelType.asString()} == ${tunnelType.asString()} = ${ttEq.toString}" )
        dirEq && ttEq
    }
